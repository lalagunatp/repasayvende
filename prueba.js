...
const globales = new Set(['if','for','while','switch','catch','return','typeof','function',
  ...
  'setTimeout','setInterval','encodeURIComponent','decodeURIComponent','isNaN','Blob','Error',
...
(async function(){
  ...
  let vueltas = 0;
  while (d.getElementById('carta') && vueltas < 40){
    d.getElementById('carta').click();
    if (d.getElementById('sabe')) d.getElementById('sabe').click();
    else break;
    vueltas++;
  }
  ok('Terminó el tema de tarjetas', !!d.getElementById('alQuiz'), vueltas + ' tarjetas');
